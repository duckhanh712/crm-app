...
class CrawlSeller extends Component<CrawlSellerProps, any> {
  ...
  componentDidMount() {
    menuStore.changeOption("13Crawl");
  ...
  requestAPI = async () => {
    ...
    if (this.props.location.search) {
      ...
      url = `/v1/crawlers/shopee/tts-shops?page=${crawlSellerStore.currentPage}&limit=${crawlSellerStore.pageSize}${approveState}&emails=${crawlSellerStore.filterEmail}${product}`;
      const resultApi = await callApi(
        url,
        "GET",
        {},
      ...
      this.props.history.push(`/seller-tts-cz?page=${crawlSellerStore.currentPage}&limit=${crawlSellerStore.pageSize}`);
    }
  };
  ...
  columns: any = [
    ...
    {
      title: "Approve State", dataIndex: "approve_state_cz",
      render: (str: string) => {
  ...
  showDetail = (str: string) => {
    this.props.history.push(`/seller-tts-cz/shop-detail?id=${str}`);
  }
