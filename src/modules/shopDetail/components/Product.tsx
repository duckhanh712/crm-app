...
class Product extends Component<any, any> {
  ...
  handleRefetch = async (id: string) => {
    ...
    const resultApi = await callApi(
      `/v1/crawlers/shopee/shop/${shopDetailStore.info._id}/product-update`,
      "POST",
      {
        "_id": id,
      },
  ...
  columns: any = [
    ...
    {
      title: "Sale price", dataIndex: "variants", key: "category",
      render: (variants: any) => (
        <React.Fragment>
          {variants[0].sale_price ? variants[0].sale_price: "null"}
        </React.Fragment>
      )
    },
    {
  ...
  requestAPI = async () => {
    ...
    url = `/v1/crawlers/shopee/converted-shops/${shopDetailStore.id}/products?page=${shopDetailStore.currentPage}&limit=${shopDetailStore.pageSizeProducts}${urlState}`;
    const resultApi = await callApi(
      url,
      'GET',
      {},
  ...
  handleApprove = async () => {
    const resultApi = await callApi(
      ...
      `/v1/crawlers/shopee/approved-shops/${shopDetailStore.id}/approved-products`,
      "POST",
  ...
  editCateAll = async () => {
    shopDetailStore.editCate = false;
    const url = `/v1/crawlers/shopee/category-products/${shopDetailStore.id}`
    const resultApi = await callApi(
      url,
      'PUT',
      {
